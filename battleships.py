...
class Application:
    ...
    def __init__(self):
        ...
        larg_fenetre = 1419
        haut_fenetre = 731
        self.main_window = pygame.display.set_mode((larg_fenetre, haut_fenetre))

        self.grid_columns = [182, 246, 311, 376, 440, 505, 569, 634, 698, 761]
        self.grid_rows = [79, 143, 207, 272, 336, 400, 464, 528, 592, 656]
        self.grid = []
        self.ships = set()
        self.ship_classes = [Carrier, Cruiser, Destroyer, Uboat]

        # Chargement et collage du ships
        shelf_carrier_vertical = CarrierVertical()
        shelf_cruiser_vertical = CruiserVertical()
        shelf_destroyer_vertical = DestroyerVertical()
        shelf_uboat_vertical = UboatVertical()

        shelf_carrier_horizontal = CarrierHorizontal()
        shelf_cruiser_horizontal = CruiserHorizontal()
        shelf_destroyer_horizontal = DestroyerHorizontal()
        shelf_uboat_horizontal = UboatHorizontal()

        Carrier.shelf_horizontal = shelf_carrier_horizontal
        Carrier.shelf_vertical = shelf_carrier_vertical
        Cruiser.shelf_horizontal = shelf_cruiser_horizontal
        Cruiser.shelf_vertical = shelf_cruiser_vertical
        Destroyer.shelf_horizontal = shelf_destroyer_horizontal
        Destroyer.shelf_vertical = shelf_destroyer_vertical
        Uboat.shelf_horizontal = shelf_uboat_horizontal
        Uboat.shelf_vertical = shelf_uboat_vertical

        self.shelf_ships = [shelf_carrier_vertical, shelf_cruiser_vertical, shelf_destroyer_vertical,
                            shelf_uboat_vertical,
                            shelf_carrier_horizontal, shelf_cruiser_horizontal, shelf_destroyer_horizontal,
    ...
    def can_drop_ship(self, first_grid_cell, ship):
        ...
        if first_grid_cell[1] + ship.size[1] > Application.GRID_HEIGHT or first_grid_cell[1] < 0:
            return False
        for i_col in range(max(0, first_grid_cell[0] - 1),
                           min(Application.GRID_WIDTH, first_grid_cell[0] + ship.size[0] + 1)):
            for i_row in range(max(0, first_grid_cell[1] - 1),
                               min(Application.GRID_HEIGHT, first_grid_cell[1] + ship.size[1] + 1)):
    ...
    def drop_ship(self, first_grid_cell, ship):
        ...
        if ship.__class__.usage_info.get_count_unplaced() == 0:
            ship.__class__.shelf_vertical.swap_image_with_alternative()
            ship.__class__.shelf_horizontal.swap_image_with_alternative()
    ...
    def main(self):
        ...
        for shelf_ship in self.shelf_ships:
            shelf_ship.rect = shelf_ship.rect.move(shelf_ship.get_shelf_position())
            self.main_window.blit(shelf_ship.get_image(), shelf_ship.rect)
        ...
        while continue_game:
            ...
            self.main_window.blit(background, (0, 0))
            for ship_class in self.ship_classes:
                self.main_window.blit(ship_class.usage_info.text, ship_class.usage_info.text_coordinates)

            if mode == Mode.COORDINATE:
                for i_shelf_ship in self.shelf_ships:
                    self.main_window.blit(shelf_ship.get_image(), shelf_ship.rect)
                ...
                for shelf_ship in self.shelf_ships:
                    ...
                    if event.type == MOUSEBUTTONDOWN and \
                            shelf_ship.rect.collidepoint(event.pos) and \
                            shelf_ship.__class__.usage_info.get_count_unplaced() > 0 and \
                            ship_chosen_for_placement is None:
                ...
                for ship in self.ships:
                    self.main_window.blit(ship.get_image(), ship.rect)
                ...
                if ship_chosen_for_placement is not None:  # a ship is being dragged
                    ...
                    self.main_window.blit(ship_chosen_for_placement.get_image(), ship_chosen_for_placement.rect)
                if event.type == MOUSEBUTTONDOWN and mouse_up:
                    mouse_up = False
                elif event.type == MOUSEBUTTONUP:
                    mouse_up = True
